...
with open('files/tags.pkl', 'rb') as f:
	labels = pickle.load(f)
with open('files/trainingdata.json', 'r') as f:
	intents = json.load(f)
...
def preprocess(sentence: str) -> list[int]:
	words = [stemmer.stem(word, to_lowercase=True) for word in nltk.word_tokenize(sentence)]
